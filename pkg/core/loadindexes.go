...
func LoadZincIndexesFromMetadata(version string) error {
	...
	for i := range indexes {
		// only load indexes assigned to us
		if !cluster.AssignCheck(indexes[i].Name) {
			continue
		}
		err := LoadZincIndexFromMetadata(version, indexes[i])
		if err != nil {
			return err
		}
	}
...
// GetZincIndexFromMetadata
// Query node can get index from metadata directly for partial query, the index is readonly.
func GetZincIndexFromMetadata(name string) (*Index, error) {
