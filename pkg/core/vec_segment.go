...
func (s *VecSegment) initVecStorePath() error {
	p := path.Join(config.Global.DataPath, path.Join(vector.VecPrefix, s.baseIndex.StorePath(), fmt.Sprintf("%04x", s.ref.Id), "stored_vec"))
...
func (s *VecSegment) openVecStoreReader() (*bluge.Reader, error) {
	...
	name := path.Join(vector.VecPrefix, s.baseIndex.StorePath(), fmt.Sprintf("%04x", s.ref.Id), "stored_vec")
...
func (s *VecSegment) getFaissIndexPath() string {
	return path.Join(s.baseIndex.StorePath(), fmt.Sprintf("%04x", s.ref.Id), "faiss")
}
